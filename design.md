...
- Processing is incremental at file level and idempotent at cleaned and mart levels
...
- Add proper orchestration and monitoring around ingestion and transformation. This could be implemented using scheduled Snowflake Tasks, notebook scheduling, or an event-driven pipeline triggered when new files arrive in the stage
- Confirm the anomaly detection business rule. The current implementation calculates anomalies independently for each turbine and day using that turbine’s own daily mean and standard deviation. This should be validated with the business and operational teams
- Define expected behaviour for incomplete daily files. If only part of a day is received, the pipeline should determine whether to wait for the complete dataset or recalculate daily statistics using both newly received and previously loaded measurements
- Agree additional data quality and business validation rules, including how missing measurements, late-arriving data and invalid sensor readings should be handled
- Replace or formalise the notebook-based transformation approach if required, for example by moving the logic into scheduled Snowflake Tasks, stored procedures, dbt models, or another production orchestration framework
- Add production-level observability, alerting, retries, failure handling and automated data quality tests
